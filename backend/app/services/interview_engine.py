...
def generate_next_question(session_data: Dict[str, Any], history_count: int) -> str:
    ...
    # Standard introductory question
    if history_count == 0:
    ...
    if interview_type == "hr":
        ...
        # Technical specific questions targeted at their matrix
...
def evaluate_response(question: str, transcript: str) -> Dict[str, Any]:
    """Evaluates the transcribed answer. (Will be replaced with deep LLM logic in Phase 4)."""
    words = transcript.strip().split()
    word_count = len(words)
    
    # Baseline scoring logic for the prototype
